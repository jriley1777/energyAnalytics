    ...
    for i in range(len(network.keys())):   
        ...
        for z in network[bn]['arcs']:     
            for k,v in network[bn]['arcs'].items():
                    labelupdate = min(network[k]['label'],(network[bn]['label']+network[bn]['arcs'][k]))
                    if labelupdate != network[k]['label']:
                        network[k]['label'] = labelupdate
                        minpath[k] = bn
                        #network[k]['label'] = network[bn]['arcs']
                        
    #print our final path
    final_path = []
    final_path.append('terminal')
    terminal = minpath[1]
    while terminal in minpath:
        if terminal == 0:
            final_path.append('start')
            break
        else:
            final_path.append(terminal)
            terminal = minpath[terminal]
            #print terminal
                    
    #return statements
    if network[1]['label'] < 1000000:
        print "The shortest path distance is " + str(network[1]['label'])
        print "The path is " + str(final_path[::-1]) #just reverses the order of the list [start:stop:step]
    else:
